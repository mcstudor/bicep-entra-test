...
$apps = 1 .. $WebAppCount | ForEach-Object {
    $appName = "test-web-app-$_"

    ## Create new user-managed identity and add it as a member to Key Vault Group
    ...
    $addId = New-AzResourceGroupDeployment @deployParams

    ## Format Key Vault secret references in environment variables
    $appSettings = @{}
    ...
    ## Deploy Web App in a deployment stack
    $templateParams = [hashtable]@{
        ...
        userAssignedResourceId = $addId.outputs.userManagedIdentityResourceId.value
        location                = $Location
    }
    $deployParams = @{
    ...
    $webApps = Set-AzResourceGroupDeploymentStack @deployParams
    @{
        webApps      = $webApps
        addId = $addId
    }
